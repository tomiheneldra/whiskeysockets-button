...
This repository shows how to send every currently known WhatsApp interactive / native flow button type using WhiskeySockets (Baileys fork) without modifying core source. The functionality is packaged and published as the npm package `@ryuu-reinzz/button-helper` which reproduces the binary node structure the official client emits so buttons render correctly for both private & group chats.
...
The enhanced functionality provided by the `@ryuu-reinzz/button-helper` package provides the missing functionality by:
