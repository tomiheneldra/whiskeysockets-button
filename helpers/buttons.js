/**
 * Enhanced wrapper utilities to enable ryuu-reinzz (Baileys fork) to send
 * WhatsApp interactive buttons / native flow messages reliably.
 *
 * Context / Rationale:
 *  - Upstream ryuu-reinzz currently lacks high‑level helpers for the new
 ...
 * ryuu-reinzz needs interactiveMessage.nativeFlowMessage structure for buttons to work
 */
/**
 * Transform a temporary high-level shape:
 *  { text, footer, title?, subtitle?, interactiveButtons: [{ name?, buttonParamsJson? | legacy }...] }
 * into the exact structure ryuu-reinzz expects in the WAMessage:
...
/**
 * Enhanced sendMessage function for ryuu-reinzz that bypasses the internal sendMessage
 ...
 *
 * @param {import('./ryuu-reinzz')} sock Active Baileys-like socket instance.
 ...
 * @throws {Error} If required ryuu-reinzz internals are unavailable.
 */
 async function sendInteractiveMessageV2(sock, jid, content, options = {}) {
  if (!sock)
  ...
  /* PATCH 1 — Dummy file creation for thumbnail */
  if ((hasThumb || shouldForce) && !content.document && !content.image && !content.video) {
    try {
      let fileBuffer, fileName = "DummyFile.txt";

      if (content.filePath) {
        fileBuffer = fs.readFileSync(content.filePath);
        fileName = content.filePath.split("/").pop();
      } else if (content.fileUrl) {
        const res = await axios.get(content.fileUrl, { responseType: "arraybuffer" });
        fileBuffer = Buffer.from(res.data, "binary");
        fileName = content.fileUrl.split("/").pop();
      } else {
        fileBuffer = Buffer.from(
          "This is a dummy file for interactive thumbnail display.",
          "utf-8"
        );
      }
      ...
      console.warn("⚠️ Gagal memuat file dummy:", e);
    }
  }

  /* PATCH 2 — Safe externalAdReply injection */
  if (hasThumb || shouldForce) {
    ...
    content.contextInfo = {
      ...
      externalAdReply: {
        ...
        title:
          ...
          `© ${global?.ownername || "RyuuBotz"} - 2025`,
        body:
          content.contextInfo?.externalAdReply?.body || "Asisten Virtual",
        sourceUrl:
          content.contextInfo?.externalAdReply?.sourceUrl ||
          "https://example.com"
      }
    };
  }

  /* Validation if interactiveButtons provided */
  if (Array.isArray(content?.interactiveButtons)) {
    ...
    if (strict.warnings.length)
      console.warn("sendInteractiveMessage warnings:", strict.warnings);
  }

  /* Conversion */
  const convertedContent = convertToInteractiveMessage(content);
  const { errors: contentErrors, warnings: contentWarnings, valid: contentValid } =
  ...
  if (!loaded)
    throw new InteractiveValidationError("Missing baileys internals", {
      ...
      errors: [
        "generateWAMessageFromContent or normalizeMessageContent not found in installed packages"
      ]
    });
...
async function sendInteractiveMessage(sock, jid, content, options = {}) {
  ...
  let generateWAMessageFromContent, relayMessage, normalizeMessageContent, isJidGroup, generateMessageIDV2;
  // Attempt to load from installed baileys package (modern ryuu-reinzz fork published as 'baileys').
  const candidatePkgs = ['baileys', '@ryuu-reinzz/baileys', '@adiwajshing/baileys'];
  let loaded = false;
  for (const pkg of candidatePkgs) {
    if (loaded) break;
  ...
  if (!loaded) {
    throw new InteractiveValidationError('Missing baileys internals', {
      context: 'sendInteractiveMessage.dynamicImport',
      errors: ['generateWAMessageFromContent or normalizeMessageContent not found in installed packages: baileys / @ryuu-reinzz/baileys / @adiwajshing/baileys'],
 ...
 * Uses the enhanced sendInteractiveMessage function that bypasses ryuu-reinzz' sendMessage
 */
/**
 ...
 * omitted for clarity.
 *
 * @param {object} sock Active socket instance (from ryuu-reinzz connect).
